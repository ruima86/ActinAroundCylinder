...
int main(int argc, const char * argv[]) {
    ...
    const int N_init= 140;                        // number of filaments
    ...
    const double kappa= 0.1;                // Unit pN/nm          spring constant of crosslinkers
    const double kappa_R1= 10.0;           // Unit pN*nm          torsional stiffness of crosslinkers
    ...
    const double viscosity = 1e-5;             // pN s /nm^2 viscosity of honey
    ...
    const string geometry= "cubecylinder";       // four options : free, cube, cylinder, cubecylinder
    ...
    /******************************/
    const double TT=200;                            // Unit s              Total simulation time
...
//    ConnectionList.push_front(q); 
    ...
    for (double tt=0.0; tt<TT; tt+=dt) {
        ...
        while (it_c!=ConnectionList.end()) {
            ...
            if ((randu()<(1-exp(-R_decross*exp(Energy/E_decross/kT)*dt))) && (tt>10.0)) {
                // break the connection
                Connection* p = (*it_c);
                outfile3 << setw(20) << tt << setw(20) << p->lifetime << setw(20) << p->transEnergy << setw(20) << p->torsEnergy << endl;
        ...
        if (geometry != "free") {
          ...
          //  else
          //  	Boundaryforce(FilamentList, geometry, boxSize_cube, height_cylinder, radius_cylinder, Bkappa/10);
        }
